    ...
    <div className="flex min-w-full flex-col items-center justify-center">
      ...
      <div className="flex flex-col items-center justify-center w-full">
        {settingMenu.map((item) => (
          <div id = {item.url} key={item.title}  className={`py-20 m-20 w-[90%] h-[400px] bg-primary text-primary text-3xl ${viewSection === item.url ? "!bg-muted-foreground text-foreground" : ""}`}>
            {item.title}
          </div>
        ))}
      </div>
